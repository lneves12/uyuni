...
import com.suse.manager.reactor.messaging.LibvirtEngineMessage;
...
public class SaltReactor implements EventListener {
    ...
    private Runnable onEngineEvent(EngineEvent engineEvent) {
        return () -> {
            if (engineEvent.getEngine().equals("libvirt_events")) {
                try {
                    LibvirtEngineMessage message = LibvirtEngineMessage.create(engineEvent);
                    if (message != null) {
