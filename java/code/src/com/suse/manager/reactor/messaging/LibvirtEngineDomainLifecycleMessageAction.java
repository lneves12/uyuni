    ...
    public void execute(EventMessage msg) {
        ...
        if (message.getMinionId().isPresent()) {
            ...
            MinionServerFactory.findByMinionId(minionId).ifPresent(minion -> {
                ...
                String guid = message.getDomainUUID().replaceAll("-", "");
                guid = VirtualInstanceManager.fixUuidIfSwappedUuidExists(guid);
                ...
                if (vms.isEmpty()) {
                    ...
                    if (Arrays.asList("undefined", "stopped", "shutdown", "crashed").contains(event) &&
                        ...
                        vms.forEach(vm -> VirtualInstanceManager.updateGuestVirtualInstanceProperties(
                                vm, vm.getName(), state, vm.getNumberOfCPUs(), vm.getTotalMemory()));
                    }
                }
            });
        }
