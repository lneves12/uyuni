...
const renderFilterEntry = (filter) => {
  ...
  if (filter.state === 'ATTACHED') {
    return (
      <li className={`list-group-item text-success ${styles.wrapper} ${styles.dettached}`}>
  ...
  if (filter.state === 'DETACHED') {
    return (
      <li className={`list-group-item text-danger ${styles.wrapper} ${styles.dettached}`}>
  ...
  return (
    <li className={`list-group-item text-sucess ${styles.wrapper}`}>
      {descr}
      {filterButton}
    </li>
  );
...
const FiltersProject = (props:  FiltersProps) => {
  ...
  return (
    ...
    <CreatorPanel
      ...
      onOpen={({setItem}) => setItem(props.selectedFilters)}
